...
function createCollectionPoint(authData,data,callback){
    try {
        ...
        ' values(?,?,?,?,?,?,?,?,?,?,?,now(),now())',
        [authData.user.id, data.name, data.description, data.assigned_to, data.start_time, data.end_time, data.status, data.location, data.address_1, data.city, data.latitude, data.longitude],
        (ex, rows) => {
            if(ex){
                callback(ex);
            }
